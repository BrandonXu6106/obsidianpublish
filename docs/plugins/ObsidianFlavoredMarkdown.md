...
- `wikilinks`:If `true` (default), turns [[../features/wikilinks.md]] into regular links.
- `callouts`: If `true` (default), adds support for [[../features/callouts.md|callout]] blocks for emphasizing content.
- `mermaid`: If `true` (default), enables [[Mermaid diagrams|Mermaid diagram](../features/Mermaid%20diagrams.md) rendering within Markdown files.
...
> [!warning]
> Don't remove this plugin if you're using [[Obsidian compatibility|Obsidian](../features/Obsidian%20compatibility.md) to author the content!
