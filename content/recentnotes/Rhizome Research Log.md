...
- [[file system|File system]] [[CRDT|CRDTs]]
...
- [Live, Rich, and Composable: Qualities for Programming Beyond Static Text](https://arxiv.org/pdf/2303.06777.pdf)
  ...
  - [[composable|Composability]]: the ability to freely combine smaller programmed artifacts into larger ones, to accomplish larger goals
...
- Gave a talk about [[agentic-computing|communal computing]] at the [DWeb YVR node](https://dwebyvr.org/) that went really well
...
- Things to tie together (how do these pieces fit together?)
  - [[Overlay Network|Overlay Network]] Layer
  ...
  - Persistence and Data Layer
    ...
    - Merge semantics and CRDTs
      - Trivial merge for fact tuples is the set union operator (maybe use [[clocks#Hybrid Logical Clocks]] here too)
...
- I feel like there is a large common ground between `git`, [[CRDT|CRDTs]] and room for cross-pollination across both
...
- Had a chat with Spencer and Raymond about the future of data / folk forums which was really energizing
  ...
  - See also: [[agentic-computing|communal computing]]
...
![[content/recentnotes/images/spencer-internet-provocation.png|400]]
...
![[content/recentnotes/images/beyond-cursors-gather-town.png]]

![[content/recentnotes/images/beyond-cursors-screenshot.png]]
...
- Prototype Sketch
  ...
  - Supports a spectrum of collaboration
    ...
    - It's not just a binary of either _working on your own_ or _working with others_
      ...
      - Obvious button to “make a new version of this document” (async branching)
        - What if we did 'implicit' branching where any change makes a new branch?
          - Open question: Could we utilize [[LLMs|LLMs]] to bundle these into “commits”?
    ...
    - Presence (peripheral awareness of others)
      ...
      - Useful information at every scale
        - Coin this the _[[information scales|Engelbart Zoom]]_ which Engelbart explored in [HyperScope](https://dougengelbart.org/content/view/154/86/)
  ...
  - Flexible [[access control|Access Control]]
...
- Had a brief chat with [pvh](https://twitter.com/pvh) about this same topic and he interestingly disagreed. Speaking from empirical evidence, [[RDF]] and tuples have never really worked. It's _hard_ for people to wrap their heads around
...
- Garbage Collection
  ...
  - Synchronized GC
    - Under [[Two-phase commit|two-phase commit]], each replica will vote on whether each tombstone is still necessary.
  - QCs for GC?
- Type-level consistency guarantees?
  ...
  - Just as 'function colouring' exists as a way of distinguishing async and non-async functions, what if we could colour other sorts of [[system model|system models]]?
...
- CockroachDB Layers
  ...
  - Replication: consensus replication using [[Raft Consensus Algorithm|Raft]] across replicas
...
- Finally finished up my blog post on [[agentic-computing|Communal Computing]]! It turns out, sharp feedback leads to better writing, who would have ever thought :')
...
- It's been a week but the hype from [[bft-json-crdt|my BFT JSON CRDT project]] has finally started to cool
...
- Today, I spent a lot of time thinking about the technical architecture of Rhizome now that I have the experience of the project behind me. Updated some diagrams in [[Rhizome Proposal]] but TLDR;
  ...
  - [[UCAN]] good
  - [[CRDT|CRDTs]] instead of Raft for most things, [[CALM Theorem]] may be useful to figure out when coordination is necessary
- Open questions
...
- I've been squeezing time wherever I can to work on this silly little project. As of 11:59pm tonight, I have finished the project and fully written out a 6.2k word blog post ([[bft-json-crdt|read it here]]) on it to pair
...
- Had a random question about a paper that Kleppmann wrote and just straight up messaged him on Twitter LOL totally not expecting him to respond
  - He did! Within just a few hours and helped to confirm that I did in fact need to sign messages using [[Asymmetric Key Cryptography|asymmetric cryptography]] to prevent forgery
...
- Started writing post on [[bft-json-crdt|a BFT JSON CRDT]]
- Ran into a potential problem with message forgery...
  ...
  - We would potentially need some sort of [[Public-key Infrastructure|PKI]] assumption where the unique ID of a node is its public key and the ID is the signed digest of the message
...
- Picked up _[[Seeing like a State|Seeing Like A State]]_ again, it feels a lot more relevant to my research now for some reason
  - We can think of a [[RDF|triple store]] as a distributed and fragmented SQL database, where instead of tables with rows and value, we have entities with attributes and values.
  ...
  - Forcing ourselves into schemas make it hard to innovate
    ...
    - It inadvertently shapes what people build -- leads to easily legible/classifiable applications (see: [[digital-identity|post on digital identity and legibility]])
    - It is treading outside the map that gives us innovation
  - This would give us contextual data for app specific data
    - We can see this as analogous to context dependent personalities (again, [[context-collapse|context collapse]] bad)
- Spent a lot of time trying to optimize `bft-json-crdt` to squeeze more performance out of the base list CRDT but to no avail
  ...
  - Especially if I want to focus on something that's more like a [[RDF|triple store]], a list is kind of useless lol
  - Going to focus more on the [[Byzantine Faults|BFT]] and JSON-aspects of this project
...
- Catching up today on a bunch of talks + reading
  - Wonderful [talk by Brooklyn Zelenka](https://www.youtube.com/watch?v=mxkAAtTvcEE&t=10656s) (CTO of Fission)
    ...
    - [[CID|CIDs]] give us **global pointers** that we can all agree on (these are hard links, unbreakable)
...
- [[CAP Theorem|CAP Theorem Tradeoffs]] and [[A Certain Tendency Of The Database Community]]
- Rhizome Architecture: now with triple-stores :))
  - Root: identity + persistence layer
    ...
    - essentially a managed `DID:key` that controls a set of [[IPFS|IPFS]] nodes to pin certain things
  ...
  - Trunk: application layer
    - Data framework layer: distributed triple store
      - rust → compiled to [[WebAssembly|WASM]] for web
      - each node has its own [[RDF|triple store]] that is created from an append-only data log
...
- [A Graph-Based Firebase](https://stopa.io/post/296)
  - Turns out most modern real-time applications look something like this:![[content/recentnotes/images/modern-app-architecture.png]]
  ...
  - We can potentially emulate this using triple stores built on top of an append-only CRDT. [[Datalog|Datalog]] and triple stores have been around for decades. This also means that people have built reactive implementations.
  - Unsure if we can leverage [[CALM Theorem|CALM]] as its Datalog but _not_ monotonic (facts can be retracted)
...
- Phillip Wang's talk -- related to [[the-fools-who-dream|reflection post]]
  ...
  - Fully commit to one thing -- deep beauty in choosing, against [[optionality|optionality]]
    - How does this fit into the [[exploit explore|exploit-explore]] tradeoff?
  ...
  - Leaving space to have conviction is inherently a privilege, how might we enable local spaces of abundance (places for [[content/designnotes/play#A re-worked definition|play]]) so that more people have that privilege?
...
- Justin Glibert's talk
  ...
  - What if... [[Kademlia DHT|Kademlia-like]] XOR distance metric but for [[State Machine Replication (SMR)|SMR]]
  - Permissionlessness is an important characteristic to enable innovation and [[emergent behaviour|emergent behaviour]]
...
- Preparing workshop notes to talk about [[computer networking|computer networking + P2P]]
- Feelings rant -- I feel an odd and unusually heavy sense of impostor syndrome today. Going to write out more stuff in [[the-fools-who-dream|this blog post]] I'm going to flesh out
- Frustrated by [this video by one of the founders of the Browser Company](https://www.youtube.com/watch?v=v0160IirdL4)
  ...
  - Josh seems to be conflating [[local-first software|local-first software]] with software that is not connected to the internet
...
- Addendum: the [[CALM Theorem|CALM Theorem]] conjectures that if program state can be expressed in monotonic Datalog, it can safely use optimistic replication. If we can always express something using an immutable Merkle-DAG with occasional consensus for GC, shouldn't this work?
...
- Thinking about [[access control|access control]] and revocation. Especially for add-only data structures, how can we prove data has been deleted or removed?
...
- Root
  ...
  - The [[Merkle-DAG|Merkle-DAG]] will be anchored using IPLD, this means that hopping cloud providers is easy as everything is [[content addressed storage|content-addressed]]
  - Storage Providers
...
- Ideal [[State Machine Replication (SMR)|SMR]] algorithm properties
  ...
  - Things to figure out
    ...
    - Is it worth writing a DSL that compiles down to different host languages? This could be really useful to provide helpful compile-time checks
      - Basically to adhere to [[CALM Theorem|CALM]], we want to make it easy to write synchronization free code (similar to Rust and how it makes it easy to write GC-free code)
...
- [[CALM Theorem|CALM Theorem]] and [[CRON Theorem|CRON Theorem]]: Basically, avoid coordination where possible, it makes things slow. When we can avoid or reduce the need for coordination things tend to get simpler and faster. This theorem tell us when it is safe to avoid coordination.
...
- Notes on [[Braid HTTP|Braid HTTP]], [[Yjs|Yjs]], [[Secure Scuttlebutt|SSB]], [[OrbitDB|OrbitDB]]
...
- Notes on [[HotStuff|HotStuff]], [[HoneyBadgerBFT|HoneyBadgerBFT]]. HotStuff seems to be a really useful lens to analyze future protocols as it is a general framework for expressing [[Byzantine Faults|byzantine fault-tolerant]] [[State Machine Replication (SMR)|SMR]].
...
- A bit of a slump day. Really spent the last month just reading about consensus protocols in _partially synchronous system models_ only to discovery that what I was really looking for was consensus protocols in _completely asynchronous system models_ (which handle cases where potentially majority replicas are offline).
  ...
  - In this last paper, I found out about the [[LR Permissionless Result|LR Permissionless Result]] which was derived earlier this year in February. It rules out the possibility for deterministic consensus in a Byzantine, permissionless model, which voids my current assumptions about the right type of consensus model for Rhizome.
...
- A summer retrospective. Anson encouraged me to write a more in-depth reflection on my research processes. I mentioned on a call that I felt unhappy with my progress this summer. I think the bulk of it comes down to doing way too much reading and not enough building and producing things.
  ...
  - It feels like the attitude I'm taking towards research is one of bumping around in the dark. For the most part its enjoyable and exhilarating, finding things out for the first time.
    ...
    - This is roughly what my self-satisfaction curve looks like for self-motivated exploration: ![[content/recentnotes/images/self-exploration-satisfaction.jpeg|400]]
...
- Finished reading [[Weaving the Web|Weaving the Web]]! Probably my favourite non-fiction read so far this year. Was supposed to just write up quotes but instead wrote a 1.2k word History of the Web piece instead :')
...
- Rough notes on [[Casper FFG|Casper FFG]], [[SBFT|SBFT]]. Revising notes on [[PBFT|PBFT]]
...
- Spent some time restructuring all my notes around [[cryptography|cryptography]] to have better note and concept separation
- Reading more about [[IPFS|IPFS]], their BitSwap protocol for block exchange is a super cool case study on how to do incentive design.
...
- Reading **Weaving the Web** by Tim Berners-Lee. More thoughts on this coming soon, but tldr; it is reassuring to hear that it took almost 13 years to combine the Internet and [[hypertext|hypertext]] together to conceptually create the Web. Even then, it took a lot of trying over many years to bring adoption for something that many didn't really see as potentially revolutionary
- Reading through [[PBFT|PBFT]] paper, really trying to understand the correctness and [[liveness|liveness]] proofs
...
- Random thoughts:
  ...
  - Good furniture and architectural choices respect user agency, allowing those in the space the ability to move around at will. How might we analogize this to software? [[digital commons|Digital commons]]?
- Read through [[Holochain|Holochain]] docs which are actually quite similar to what I have in mind for Rhizome.
  - Really liked
    - Using [[RDF|RDF]] triples in a [[DHT|DHT]] to create a distributed graph database is a smart way to network the data -- feels like what semweb was supposed to be
    - Everything is self-owned and consistency of application state is maintained by storing hashes of actions to a global [[DHT|DHT]] which allows for peer accountability
  - Things that I think are unaddressed
    ...
    - How important is global data-witnessing? Why do we need social pressures for this when we can do this using [[cryptography|cryptography]]?
...
- Learned a lot about [[Network Theory|Network theory]]
  ...
  - Also notes on [[cascading failures|cascading failures]], interesting to note that sometimes the most effective way to stop a failure is to prematurely kill edges and nodes (e.g. burning parts of the forest ahead of a forest fire to clear debris in a controlled manner)
- Stressed about what to do for the upcoming gap term/year!!
  ...
  - Other thoughts: in my [[2021|Letter to my Future Self]], I mentioned I wanted to reach deep focus in whatever work I do and have the resources to be able to choose the work I find enjoyable. I'm not going to half-ass do whatever, so it will either be full-send research or full-send finishing school.
  - Options
...
- Gordon Brander deep-dive today... more thoughts on the [[inevitability of centralization|inevitability of centralization]] and [[credible exit|credible exit]]
...
- Content-addressing feels like a viable alternative to how loading resources works on the internet today.
  - Address-based addressing relies on a central registry to figure out where things are. [[content addressed storage|Content addressed storage]] (e.g. [[CID|CIDs]]) _decouples_ the data from the origin. If you know what the hash is, you can request the original file, irrespective of where the file actually lives.
...
- On a whim, read this awesome [Patreon post](https://www.patreon.com/posts/ratcheting-in-47976114) by Andy Matuschak
  - _Ben Shneiderman, a pioneering human-computer interaction researcher, offers this charming schematic for research project design in The New ABCs of Research. He calls it the “two parents, three children” pattern._
    ![[research project pattern.png|500]]
...
- Finished up the [[digital-identity|identity piece]]! Cent and B from Metagov gave some very good advice and clarifying feedback on the piece.
...
- Polishing up [[digital-identity|identity piece]]. I've worked on it enough that I'm starting to feel ick just touching it but I'm happy that I've thought about this deeply. Implications for Rhizome as a whole:
  - Self-sovereignty seems useful for agency _if implemented in ways that don't force legibility_
    - e.g. be careful about [[Verifiable Credential|VCs]] without zkSNARKS
...
- Slowly reaching a place where I'm happy with the direction of this piece on identity, framing it more around 3 modes of thinking about [[digital-identity|identity]].
...
- I've been trying to write down some more cohesive thoughts around [[identity|identity]] for the past two days and running into a block where I'm struggling to articulate why and how real world identity and digital identity differ
...
- Currently on a two-day writing retreat with Belinda, Athena, and Vincent. It's been such a good mix of sight-seeing and focused writing.
  ...
  - Some good time away from purely technical reading meant I had time to think more about identity. More thoughts around [[digital-identity|verb based identity]]
...
- Spent a lot of time just reading today, a lot of different scattered blogs that I've been meaning to get to. [One link that was sent](https://generative-identity.org/human-identity-the-number-one-challenge-in-computer-science/?curius=1294) in the Metagov Slack particularly stood out to me though. It was on human identity and, more specifically, a critique of specifically [[Self-sovereign Identity (SSI)|SSI]]
  - Long read but I think it captures a lot of my thinking around why I think [[digital permanence|digital permanence]] is scary (and why I've been thinking about relational notions of identity!) TLDR;
...
- Going to read more about [[causal tree|causal trees]] as a way of understanding more basic forms of [[CRDT|CRDTs]] that value readability over correctness
...
- Finished up CRDT implementation collection over at [[CRDT Implementations|CRDT Implementations]].. I feel like I'm getting a better grasp at how to write op-based CRDTs but less so for state-based

### July 11th

- Seems like there are a lot of open research questions in CRDTs that I could plausibly spend _years_ working on (e.g. undo operations in CRDTs, encrypted CRDTs using homomorphic [[encryption|encryption]])
- I need to read more about this but it seems like most traditional consensus algorithms require synchronicity from all nodes for them to be considered honest. I wonder how we can reconcile this methods like CRDTs that allow for more asynchronous forms of consistency
  - Is it possible to take advantage of the partially synchronous [[system model|system model]] and having CRDT-like behaviour in async modes and Raft/Paxos-like behaviour during synchronous periods for compaction
...
- Do people care about data ownership and data agency?
  ...
  - But as a counterpoint, if you ask anyone on the street whether they would be comfortable sharing their entire browsing history right there on the spot, my bet is that ~95% of people will say no
    ...
    - How much does the average person value their [[privacy|privacy]]?
...
- Why would companies care about this model of computing?
  ...
  - [[GDPR|GDPR]] compliance built-in, users have freedom to manage their own data
...
- Various notes on CRDTs
  - Seem to require [[consensus|consensus]] for state compaction
  - Learning about [[clocks#Hybrid Logical Clocks|HLCs]] and maintaining [[causality#Causal Order|causal order]] in CRDTs

### July 8th

- Learning more about [[CRDT|CRDTs]], [[Order theory|Order Theory]]
- I got my first email from a mutual which was along the lines of "help, I'm stuck in leetcode hell, how do I escape and do other things?"
  ...
  - Here was the email response I sent: ![[Screenshot 2022-07-08 at 4.08.41 PM.png]]
...
- Finished up Tim Roughgarden's lectures on the foundations of blockchain which had some really useful theoretical details on traditional [[consensus|consensus]] mechanisms which definitely solidified my understanding
...
- Read a really good paper on [[neutrality|neutrality]] and learned about the [Data Transfer Project](https://datatransferproject.dev/)
...
![[content/recentnotes/images/IMG_1805.png|???]]

### July 5th

- Finalizing notes on [[Tendermint|Tendermint]] and wondering if I should switch out [[Raft Consensus Algorithm|Raft]] for it. How valuable is [[Byzantine Faults|BFT]] anyways? Do we assume nodes are prone to potentially malicious takeover?

### July 2-4th

- An 'aha' moment caught in 4k... watch me try to figure out why asynchronous and partially synchronous [[system model|system models]] aren't the same thing (s/o Sebastien for being so kind and patient). This was super satisfying!

![[content/recentnotes/images/on-async-partially-sync-models.png]]

### July 1st

- Internet went out today halfway through watching lectures :((
  - Spent a bunch of time just reading books + thinking
- More notes from Tim Roughgarden's foundation course on [[Public-key Infrastructure|PKI]], [[Byzantine Broadcast|BB]], impossibility theorems, etc.
...
- Sebastien sent a [YouTube playlist](https://www.youtube.com/watch?v=KNJGPI0fuFA&list=PLEGCF-WLh2RLOHv_xUGLqRts_9JxrckiA&index=1) on the foundations of [[blockchain|blockchains]] that have some sections which seem highly relevant. Slowly making my way through these
...
- Thinking it might be good to do a general overview of [[CRDT|CRDTs]] before delving any further
...
- More research on [[CouchDB|CouchDB]] and other database replication mechanisms to see what I can learn from it
...
- Reading about [[Hyper Hyper Space|Hyper Hyper Space]], doesn't seem to place a big deal of emphasis on finality which seems important for a large chunk of applications.
- Open questions:
  - Append-only log or append-only [[Merkle-DAG|Merkle-DAG]]? Leaning more towards log still for easy understandability + debug even though Merkle-DAGs are more expressive (and battletested in [[blockchain|blockchains]] and `git`)

### June 20th - 23rd

- Reading about [[file system#Virtual Distributed File System|VDFS's]] (specifically Alluxio) and
- Open Questions
...
- Generally find framing it in terms of net neutrality but applied to data gets a lot of people excited about it, as well as meaningfully explaining + differentiating from Tim Berners-Lee's [[Solid|Solid]] project and how Rhizome focuses on addressing main retro points from major p2p protocols.

### June 14 - 15th

- Mostly trying to answer questions around how [[decentralized marketplace|decentralized marketplaces]] for demand work, looking at Golem and Orchid
- Lots of moving around (moved from Tempe to SF, about to head to Interact retreat!)

### June 13th

- Rough research notes and open questions on [[DID#DWN|DWNs]]
...
- Permissions request
  ...
  - Potentially using [[UCAN|UCANs]]
- Open questions
  - How does DID ownership work? what is it pinned to? is IPFS sufficient?
    - TLDR; DID needs to be generally anchored to something. Notes on [[Sidetree|Sidetree]], a backend agnostic DID persistence mechanism
...
- Spicy day today... Jack Dorsey just announced TBD working on Web5, supposedly an extra decentralized web platform (https://twitter.com/jack/status/1535314738078486533)
  ...
  - Very similar to [[WebID|WebID]] except anchored on bitcoin (lots of interesting stuff using [[Sidetree|Sidetree]])
...
- Lots of research, mostly around [[FOAF|FOAF]], [[LDP|LDP]], [[RDF|RDF]]
- Looked more into [[decentralized marketplace|decentralized marketplaces]] like Raiden and Orchid to see how they handle payments
...
- Learned more about underlying datastructures of [[IPFS|IPFS]] including [[CID|CIDs]]
  - Potential for interop between IPFS and [[DID#Creating DIDs using IPLD|DID Documents]]?
- More notes on [[DHT|DHTs]] and [[Kademlia DHT|Kademlia]] in particular
...
- Notes on [[DID|DID]] which seems particularly applicable to the notion of identity + identity documents
...
- More reading + notes in [[decentralization|decentralization]], [[authorization|authorization]], and [[federation|federation]]. Notable readings:
...
- More meditations on identity and data
  ...
  - Learned about the concept of [[petname|petnames]] in more depth today and there's a really cool way of thinking about identity here perhaps
...
- Mostly writing up recent learnings and incorporating them into the [[Rhizome Proposal|research proposal]]... lots of words today
...
![[content/recentnotes/images/rhizome-may-6.jpeg]]

### May 5th

- Finishing up [[distributed systems#Martin Kleppmann's Course|Martin Kleppmann's Course on Distributed Systems]]
...
- What if... Rhizome had built in mechanisms for managing 'branches'
  ...
  - To make a collaborative doc you can 'fuse' or 'join' branches together temporarily to sync them with each other
    - What if we made something on top of `git` like this that actually functions on a syntax level rather than a character level... one for the [[idea list|idea list]]
...
- Agreeing on what operations a [[CRDT|CRDT]] can perform still seems to be difficult ([see 1hr into this talk](https://youtu.be/Qytg0Ibet2E?t=3665))
  - Possible room for data lensing on public schemas to be useful here

### May 4th

- Skimming [[distributed systems#Martin Kleppmann's Course|Martin Kleppmann's Course on Distributed Systems]]
  - Really good foundation to work off of
  - Learned about differences between physical and logical [[clocks|clocks]] and realizing that `miniraft` should probably use some sort of [[clocks|logical clock]] rate

### May 3rd

- Read about more [[NAT#Efficacy|NAT traversal and holepunching efficacy]], turns out hole punching is just not as reliable as I thought it was
- Compared more traditional consensus algorithms like [[Raft Consensus Algorithm|Raft]] to [[Solana|Solana]].
...
![[content/recentnotes/images/rhizome-may-3.jpeg]]

### May 2nd

- Mostly reading about [[Raft Consensus Algorithm|Raft]] consensus algorithm today and understanding how it works
  - Always wondered how these consensus algorithms deal with bad actors -- turn out they don't! That's where [[Byzantine Faults|BFT]] comes in
  - Seems to be promising for replicating between trusted peers (potentially applicable)
- Starting a very minimal stripped down implementation of [[Raft Consensus Algorithm|Raft]] in Rust I am nicknaming `miniraft`. Code [here](https://github.com/jackyzha0/miniraft) (but will most likely be private until it is done).

### May 1st

- Settling back into home, general research reading + writing [[Rhizome Proposal|the proposal]]
- Read various papers
  - [[internet computing#Changing an entrenched internet|Changing an entrenched Internet]]
  - [[mechanism design|Mechanism Design]]
  - [[Raft Consensus Algorithm|Raft]]
- Learned about the basic premise of [[Self-sovereign Identity (SSI)|SSI]]
