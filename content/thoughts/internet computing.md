...
## Network [Protocol](Protocol.md) Stack (from most abstract to least)

Each layer takes data from above adds header information to create new data unit passes new data unit to layer below

1. [Application Layer](Application%20Layer.md) ([HTTP](HTTP.md))
2. [Transport Layer](Transport%20Layer.md) ([TCP](TCP.md), [UDP](UDP.md))
3. [Network Layer](Network%20Layer.md) ([IP Address](IP%20Address.md)) -- this is the 'thinnest' part of the network stack!
4. [Link Layer](Link%20Layer.md)
5. [Physical Layer](Physical%20Layer.md)

## Hubs, Switches, and Routers

1. Hub - broadcasting through cloning bits ([physical layer](Physical%20Layer.md))
   1. Simplest and cheapest way to create a network
   2. Lots of unnecessary traffic
   3. Other people can see your traffic
2. Switch - hub but it knows where other hosts are for direct addressing ([link layer](Link%20Layer.md))
...
3. Router - glue that ties networks together ([network layer](Network%20Layer.md))
   ...
   3. Modern routers can also perform
      1. Network address translation ([NAT](NAT.md))
      2. Assigning [IP addresses](IP%20Address.md) to hosts using [DHCP](DHCP.md)
      3. Broadcast WiFi signal

## Error Correction

Methods for [fault tolerance](fault%20tolerance.md) in data transmission
...
- We can use a sliding window to represent the 'window' of all bits that are in flight
  - Although, better version of this is to just use a proper protocol like [TCP](TCP.md)
