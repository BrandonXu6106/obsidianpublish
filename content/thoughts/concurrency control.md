...
- **Multi-version concurrency control (MVCC)** - maintaining multiple versions of data so readers don't block writers
	- The most common [[isolation.md|isolation]] level implemented with MVCC is snapshot isolation. With snapshot isolation, a transaction observes a state of the data as of when the transaction started.