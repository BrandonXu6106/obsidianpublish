---
title:
  - There is considerable debate over how we should react if we detect a signal from an alien civilization.
date: 2025-08-24
tags:
  - IELTS/vocabulary
  - 新东方
  - incubate
  - 100个句子记完7000个雅思单词
draft: false
enableToc: true
---
> [!info] sentence
> - There is considerable debate over how we should react if we detect a signal from an alien civilization.
> - 如果我们探测到了来自外星文明的信号，我们应该如何回应是一个备受争议的问题。

## 1. 句子解析
>==There is [[thoughts/words/considerable|considerable]] [[thoughts/words/debate|debate]] over== <u>how we should react</u> if we detect a signal from an alien civilization.
>如果我们探测到了来自外星文明的信号，我们应该如何回应是一个备受争议的问题。
- **主句**: There is considerable debate over sth 某事备受争议
- **debate**：
	- debate on/about/over sth 
	- there is ==a== debate（==c==, u）→ there is considerable debate（==u==）
- **语法**：从句做名词→名词性从句(宾语从句)
- **名词性从句**：
    - ★★★用<u>陈述语序</u>: 主语+助动词/动词
	    - How should we react?（疑问语序）→  How we should react（陈述语序）
    - 应用范围: 宾语从句、主语从句等都属于名词性从句

## 2. 主题归纳-太空探索
- spacecraft n.航天器，宇宙飞船
	- space（太空）+craft（技术）→ 太空技术 → 航天器
- space probe 太空探测器
- manned sapce flight 载人航天飞行
- [[thoughts/words/planetary|planetary]]`/ˈplænəteri/` adj. 行星的
- planetary system 行星系
- solar adj. 太阳的；太阳能的 
- habitable`/ˈhæbɪtəb(ə)l/` adj. 可居住的
	- 同义词：**in**habitable，livable
	- 反义词：**unin**habitable ,unlivable
- habitat`/ˈhæbɪtæt/` n. 生长地，栖息地
- expense n. 花钱，开销
	- worth the expence 物有所值
	- spare no expense 不遗余力
	- at great/considerable/enormous expense
	- cut down on expense 削减开支
- costly adj, 昂贵的，代价高昂的
	- costly jewelry 价格昂贵的珠宝
	- costly victory 代价惨重的胜利
	- costly mistake 损失重大的错误
	- ★★ 引申义用法更显语言的高级性


>==There is [[thoughts/words/considerable|considerable]] [[thoughts/words/debate|debate]] over== <u>how we should react</u> if we detect a signal from an alien civilization.
>如果我们探测到了来自外星文明的信号，我们应该如何回应是一个备受争议的问题。


