...
tags:
  - seed
---
For the purposes of permission checks (whether a process on behalf of a user is able to do a specific thing), traditional [[Unix.md|Unix]] implementations distinguish two categories of processes:
...
We can get even more granular with `seccomp` which can grant/filter/block specific syscalls via an allow list / deny list model. `seccomp-bpf` is an extension to `seccomp` that allows filtering of system calls using a configurable policy implemented using [[Link Layer#Berkeley Packet Filter](Link%20Layer.md) rules.

See also: [[access control|access control](access%20control.md)
## Namespaces
...
Linux containers are based on top of Linux namespaces (“which resources to isolate per container”) and [[cgroups.md|cgroups]] (“how much of each resource to allocate to each container”).  As such they can be thought of as running native processes directly on top of the kernel but each container only having a local view of the OS inside it. For e.g. a process in a container with a process namespace, may think of “init” as its process with PID1 but outside the container this “init” is like any other user-space process on the system. Similarly, a process in a container with a mount namespace may access “root” (“/”) but in reality it may be any arbitrary path on the system.

See also: [[docker.md|docker]]
