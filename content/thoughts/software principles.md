...
> [[A Pattern Language|A Pattern Language](A%20Pattern%20Language.md) for developing software
...
1. [[local-first software|Local first](local-first%20software.md). Decentralized/[[distributed systems|distributed systems](distributed%20systems.md) second. Make self-hosting easy
2. There is a latent cost to new features: [maintenance](maintenance.md)
3. Heuristics should only be used where determinism is infeasible
4. Apps should embrace gradual enrichment, [[composable.md|composing]] smaller features to form more complex ones when necessary
5. Dump the parts bucket onto the floor. Make it obvious what tools the user has at their disposal to make the most out of the software ([[constructionist.md]] learning)
6. If a system is to serve the creative spirit, it must be entirely comprehensible to a single individual
7. Any system should provide a uniform means for referring to the things in its universe. In fact, an even stronger statement is that systems should be designed around a powerful metaphor that can be uniformly applied in all areas. Only from this can complex systems [[emergent behaviour|emerge](emergent%20behaviour.md)

See also: [[programming models](programming%20models.md)
...
3. **Low floor, high ceiling.** The program should be extremely accessible by default, but allow for very complex functionality. Toward this, I've been trying an expert mode toggle, like taking off the training wheels. (see also [[notation.md#Cognitive Dimensions of Notations|cognitive dimensions of notation]])
