...
We can imagine WASM modules as functions that execute some behaviour. If we [[content addressed storage|content address]] it, we now have a consistent way of referring to the same computation. We can imagine a particular WASM module call with a set of parameters as a suspended closure that is deterministic[^1].
