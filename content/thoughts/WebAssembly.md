...
We can imagine WASM modules as functions that execute some behaviour. If we [[content addressed storage|content address](content%20addressed%20storage.md) it, we now have a consistent way of referring to the same computation. We can imagine a particular WASM module call with a set of parameters as a suspended closure that is deterministic[^1].
...
This actually gives us superlinear results as we increase concurrency (opposite of what the [[Universal Scaling Law](Universal%20Scaling%20Law.md) says! [[BitTorrent.md]] does this too)
