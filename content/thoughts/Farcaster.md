...
1. On-chain registry for identity registration (like [[DID#High level overview|DID VDRs]]). Table of `username`, `address`, and `host_url`
...
There are two options for hosting: self-hosting and using a managed host (like Gmail does for email and Github does for [[git|Git]]).
