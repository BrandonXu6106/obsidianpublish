...
![[p2p internet.png](images/p2p%20internet.png)
...
Nature of course, has its way of selecting for resiliency. In ecology, there is a concept called [[r-K Selection theory|r-K Selection Theory](r-K%20Selection%20theory.md) related how certain species trade off between quantity and quality of their offspring. It notes that there are predominantly two types of species:
...
![[r vs K-selected species.png](images/r%20vs%20K-selected%20species.png)
...
![[network stack.png](images/network%20stack.png)

When you request a website or make an API call, you go down all these layers and then back up at the other end.

- [[Application Layer|Application Layer](Application%20Layer.md) -- where applications can access the network services using protocols like [[HTTP.md|HTTP]].
- [[Transport Layer|Transport Layer](Transport%20Layer.md) -- ensures data arrives in order, recovers lost data by retrying, sends data to the right process on your machine
- [[Network Layer|Network Layer](Network%20Layer.md) -- routes packet through routers to destination machine
- [[Physical Layer|Physical Layer](Physical%20Layer.md) -- connecting individual machines together, transferring the actual bits
...
![[facebook communication.png](images/facebook%20communication.png)
...
When we refer to [[peer-to-peer.md|peer-to-peer]] today, we don't mean the underlying networking stack, but rather on the Application level. How can we bypass going through servers to facilitate all of our actions and instead connect directly with our peers?
...
A lot of my [[Rhizome Proposal|independent research](Rhizome%20Proposal.md) is around how to make it easier to write peer-to-peer software and make it possible to do the things client-server models are good at in these peer-to-peer contexts (e.g. data persistence).
...
In the face of this uncertainty, we still need some way to come to some shared understanding of what the 'state' of the application is. In literature, this is called the [[State Machine Replication (SMR)|state machine replication](State%20Machine%20Replication%20(SMR).md) problem.
...
More recently, researchers found another way of doing this is using a specific type of replicated data structure called a [[CRDT.md|commutative replicated data type]] or CRDT. This uses mathematical properties of operations to guarantee that even if messages are received out of order, they will all eventually converge to the same result. CRDTs are at the base of a lot of popular 'shared type' libraries like [[Hypercore.md|Hypercore]] or [[Yjs.md|Yjs]], which make it really easy to use JSON-like data-types in your code that automatically receive updates from other peers.
